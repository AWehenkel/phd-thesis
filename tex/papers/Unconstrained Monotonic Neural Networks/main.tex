...
\begin{align}
    ...
    %&= \int^x_0 f(t; \mathbf{\psi}) \dt + \beta ,\label{UMNN-1}
\end{align}
...
\begin{align}
    ...
    %&=\log p_Z(\mathbf{g}(\mathbf{x}; \mathbf{\theta}))  + \log\left|\prod^d_{i=1} f^i(x_i, \mathbf{h}^i(\mathbf{x}_{1 : i-1}))\right| \nonumber \\
...
% \begin{align}
%     \mathbf{z}_{1:k} = \mathbf{x}_{1:k}\quad \text{and}\quad
%     \mathbf{z}_{k+1:d} = e^{\mathbf{\sigma}(\mathbf{x}_{1:k})} \circ	 \mathbf{x}_{k+1:d} + \mathbf{\mu}(\mathbf{x}_{1:k}),
% \end{align}
% where $\mathbf{\sigma}$ and $\mathbf{\mu}$ are two unconstrained functions $\mathbb{R}^{d-k} \rightarrow \mathbb{R}^{d-k}$ and $\circ$ symbol is used to denote the Hadamard product. %The Jacobian of such transformation is lower triangular with the $k$ first elements of the diagonal equal to one and the $k-d$ last elements equal to $e^{\sigma(\mathbf{x}_{1:k})}$.
...
Experiments were carried out using the same integrand neural network in the UMNN component -- i.e., in Equation~\ref{eq:umnn}, $f^i = f$ with shared weights $\mathbf{\psi}^i = \mathbf{\psi}$ for $i \in \{1, \hdots, d\}$.
...
\begin{align}
    %L(\mathcal{B}, \mathbf{\theta}) = \sum^{N}_{i=1} \mathcal{L}(\mathbf{\theta}; \mathbf{x}^i).
    L(\mathbf{\theta}) = - \sum_{n=1}^{N}\left[ \log p_Z(\mathbf{g}(\mathbf{x}^n; \mathbf{\theta})) + \sum^d_{i=1} \log f(x_i^n, \mathbf{h}^i(\mathbf{x}_{1 : i-1}^n))\right]. \label{eq:loss_toy}
\end{align}
...
%\vspace{1em}
    ...
    \begin{tabular}{l l c c c c c c }
        ...
        & TAN - \cite{TAN}& $\underline{-0.60}_{\pm.01}$ & $\mathbf{\underline{-12.06}}_{\pm.02}$ & $\mathbf{\underline{13.78}}_{\pm.02}$ & $\underline{11.01}_{\pm.48}$ & $\mathbf{\underline{-159.80}}_{\pm.07}$ & $\underline{1.19}$\\\hline

        \multirow{3}{*}{(c)}
        & NAF - \cite{NAF}& $-0.62_{\pm.01}$ & $-11.96_{\pm.33}$ & $15.09_{\pm.40}$ & $\mathbf{\underline{8.86}}_{\pm.15}$ & $-157.73_{\pm.30}$ & - \\
       ...
       % \meanstd{9.512812}{9.831732}{9.656743}
        ...
        & $\mathbf{\underline{1.13}}_{\pm.02}$ %\meanstd{1.102028}{1.143692}{1.137857}
        \\ \hline
    \end{tabular}
...
%\begin{table}[H]
    ...
    %\begin{tabular}{l|c c c c}
        %Dataset &
        %\begin{tabular}{cc}\multicolumn{2}{c}{\textbf{MNIST}} \\ -ELBO & NLL\end{tabular} & %\begin{tabular}{cc}\multicolumn{2}{c}{\textbf{Freyfaces}} \\ -ELBO & NLL\end{tabular} & %\begin{tabular}{cc}\multicolumn{2}{c}{\textbf{Omniglot}} \\ -ELBO & NLL\end{tabular} & %\begin{tabular}{cc}\multicolumn{2}{c}{\textbf{Caltech 101}} \\ -ELBO & NLL\end{tabular} \\
        %\hline
...
%\begin{align}
%    L(\mathbf{x};\theta) = \log p(\mathbf{x}) - D_{KL}(q(\mathbf{z}|\mathbf{x})\| p(\mathbf{z}|\mathbf{x})),
%\end{align}
%A Variational Auto Encoder is an Auto Encoder trained with variational inference. To do so the observations are assumed to be drawn from the marginal of a deep latent model. in which the encoded representation of the sample $\mathbf{x} \sim p(\mathbf{x})$ is a random variable that follows a distribution $p(\mathbf{z}|\mathbf{x})$ called the posterior. In this setting the role of the output of the decoder is modelled by a random variable $\hat{\mathbf{x}} \sim p(\hat{\mathbf{x}}|\mathbf{z})$. As for classical Auto Encoder, VAE aims at modelling a generating procedure that maps latent variable to samples from the dataset. Thanks to the use of a latent posterior distribution, VAE expresses this problem as an inference problem in which we want to maximise
