...
The resulting model is trained by maximizing the likelihood of the data $\{\mathbf{x}^1, ..., \mathbf{x}^N\}$. NFs can also be used for data generation tasks while keeping track of the density of the generated samples such as to improve the latent distribution in variational auto-encoders \citep{NF}.
...
Coupling conditioners~\cite{RealNVP} are another popular type of conditioners used in normalizing flows. The conditioners $\mathbf{c}^i$ made from coupling layers are defined as
\begin{align*}
    \mathbf{c}^i(\mathbf{x}) =
    \begin{cases}
