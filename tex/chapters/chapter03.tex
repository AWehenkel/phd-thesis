...
\end{align}
under mild conditions. The symbol $t$ denotes the time; it starts from $0$ with structured data and ends in $1$ with noise. We can use stochastic gradient descent to optimise this term, the encoder entropy, and reconstruction terms jointly. In order to evaluate \Cref{eq:CE_continuous_diffusion_bis}, we first encode the sample ($q(\bm{z}_0 \mid \bm{x})$), draw $t\sim \mathcal{U}\left[0, 1\right]$ and the corresponding $\bm{z_t} \sim q(\bm{z}_t \mid \bm{z}_0)$ that follows a known Normal transition kernel $\mathcal{N}\left( \bm{\mu}_t(\bm{z}_0), \sigma_t^2 I \right)$. \Cref{eq:CE_continuous_diffusion_bis} is a score matching loss; it resembles the $\operatorname{ELBO}$ objective used to train discrete-time models, which is what we use to replace the CE. There remains a surprising difference between the equation (12) in the paper and \Cref{eq:CE_continuous_diffusion_bis}. In our case, the inverse variance $\frac{1}{\sigma_t^2}$ weights each term while \Cref{eq:CE_continuous_diffusion_bis} weights with $g(t)^2$ which is supposed to have a similar meaning to $\sigma_t^2$ in the discrete case. \citet{vahdat2021score} suggest moving away from this weighting to obtain better results in practice. We wonder if there is a deeper explanation.
