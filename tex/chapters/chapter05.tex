...
The paper focuses on normalizing flows as an important application of the newly introduced monotonic transformation. However, monotonic transformations can help with many other challenges in machine learning. Among them, model calibration is arguably an essential issue in machine learning. Uncalibrated models provide biased confidence scores; model calibration corrects this bias to better estimate the model's confidence. Recently, \citet{gruber2022trustworthy, deycalibrated, rahimi2020intra} relied on unconstrained monotonic neural networks to parameterise the calibration layers in diverse settings. Another application is to induce a monotonic relationship between a subset of the input variables and the model's output. For example, \citet{yurk2021county} study the effect of business closures on the speed of propagation of COVID-19 with ML models. They enforce a monotonic relationship between the tightness of public policies and the observed reproduction rate in their models with an unconstrained monotonic neural network.


- Impact on the NFs. Layer used by Graph flows, important to be able to model multimodal distributions. Showed that inversion of monotonic models is possible. And we can also compute the derivative with respect to this operation if it is really needed. This is something used by stochastic flows.

Since its publication in December 2019, this paper has received
