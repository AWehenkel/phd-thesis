...
Formally we aim to recover the Bayes optimal classifier, that is for all $\mathbf{y})$ to be able to return the posterior distribution $P(C \mid \mathbf{y})$. This gives the following optimisation problem:
\begin{align}
  ...
  &= \argmin_{\bm{\theta}} -\mathbb{E}_{P(\mathbf{y} \mid C=0)}\left[ \log \left( \frac{P_n(\mathbf{y})}{P_n(\mathbf{y}) + P_{\bm{\theta}}(\mathbf{y})} \right)  \right] -\mathbb{E}_{P(\mathbf{y} \mid C=1)}\left[ \log \left( \frac{P_{\bm{\theta}}(\mathbf{y})}{P_n(\mathbf{y}) + P_{\bm{\theta}}(\mathbf{y})} \right) \right],
\end{align}
which is the standartd cross-entropy loss applied to a classifier that resembles to \Cref{eq:CL} parameterised by an EBM.
