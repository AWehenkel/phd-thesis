...
\end{align}
We can replace $P(\mathbf{y})$ by the EBM $P_{\bm{\theta}}(\bm{y})$ and solve the corresponding classification problem as a function of the EBM's parameters $\bm{\theta}$ and of the normalizing constant $\alpha \triangleq Z(\bm{\theta})$. This gives the following optimisation problem:
\begin{align}
  \bm{\theta}^\star &= \argmin_{\bm{\theta}} \mathbb{E}_{P(\mathbf{y})}\mathbb{KL}\left[ P(C \mid \mathbf{y}) \Vert P_{\bm{\theta}}(C \mid \mathbf{y}) \right]\\
  &= \argmin_{\bm{\theta}} -\mathbb{E}_{P(\mathbf{y})}\mathbb{E}_{P(C \mid \mathbf{y})}\left[ \log P_{\bm{\theta}}(C \mid \mathbf{y}) \right]\\
  &= \argmin_{\bm{\theta}} -\mathbb{E}_{P(C)}\mathbb{E}_{\mathbf{y} \mid P(C)}\left[ \log P_{\bm{\theta}}(C \mid \mathbf{y}) \right]\\
  &= \argmin_{\bm{\theta}} -\mathbb{E}_{P(C)}\mathbb{E}_{\mathbf{y} \mid P(C)}\left[ \log \left( \frac{P_n(\mathbf{y})}{P_n(\mathbf{y}) + P_{\bm{\theta}}(\mathbf{y})} \right) \right],
\end{align}
which is the standartd cross-entropy loss applied to a classifier that resembles to \Cref{eq:CL} and is parameterised by an EBM.
