...
\parbox{\textwidth}{%
  ...
  \raggedleft\MakeUppercase{Immanuel Kant, as paraphrased by Maria Konnikova}\par%
}}
...
We want to learn a model that can predict the conditional distribution $P(Y=y\mid X=\bm x)$ that I like a wine with features $\bm x \in \mathbb{R}^d$, where $y \in \mathbb{R}$ is a value that represents y aphynity with the wine, it is larger if I like ($\rightarrow \infty$) the wine or smaller ($\rightarrow -\infty$) if not. We may consider a neural network that is parameterized by $\bm \theta$ and represents a parametric density function conditioned on an input $\bm x$, $f_{\bm \theta}(\cdot; \bm x): \mathbb{R} \times \mathbb{R}^{d} \rightarrow \mathbb{R}^+$.
Learning aims at summarising the information in the data to perform the task of interest, that is to predict $P(Y \mid X, \mathcal{D})$.

We state that $f_{\bm \theta}$ represents the conditional distribution $P(Y\mid X, \bm \theta)$. And we suppose this set of parameters is expressive enough to summarize all information about the ``true'' distribution $P(Y\mid X)$ contained in the data $\mathcal{D}$. Then what we need for making new predicitions based on the data observed $\mathcal{D}$ is 1) To condition our predictions on the data $\mathcal{D}$ and; 2) To marginalize with respect to the unknown ``true'' parameters $\bm \theta$. We also exploit the conditional independence $ Y \indep \mathcal{D} \mid X, \bm \theta$
\begin{align}
...
\textcolor{red}{Define what is inductive bias.}
%
% Things to mention:
...
Fixing the topological ordering in advance relaxes \Cref{eq:learn_structure} into a simpler problem for which greedy algorithms exist. In this case, the solution's quality depends on the chosen ordering. In addition to its intractability, the second limitation of the formulation in \Cref{eq:learn_structure} is the lack of prior knowledge. A partial solution is to reduce the search space $\mathcal{G}$ to the structures we consider plausible. Expressing more subtle priors over graphs and handling them efficiently in structure learning is still an open research question.
