...
We want to learn a model that can predict the conditional distribution $P(Y=y\mid X=\bm x)$ that I like a wine with features $\bm x \in \mathbb{R}^d$, where $y \in \mathbb{R}$ is a value that represents my aphynity with the wine, it is larger if I like ($\rightarrow \infty$) the wine or smaller ($\rightarrow -\infty$) if not. Learning aims at summarising the information in the data to perform the task of interest, that is to predict $P(Y \mid X, \mathcal{D})$.

Our class of models is a neural network that is parameterized by $\bm \theta$ and represents a parametric density function conditioned on an input $\bm x$, $f_{\bm \theta}(\cdot; \bm x): \mathbb{R} \times \mathbb{R}^{d} \rightarrow \mathbb{R}^+$. This means $f_{\bm \theta}$ represents the conditional distribution $P(Y\mid X, \bm \theta)$. In this context, we suppose the parameters $\bm \theta$ are expressive enough to summarize all information about the ``true'' distribution $P(Y\mid X)$ contained in the data $\mathcal{D}$. Then learning requires 1) To condition our predictions on the data $\mathcal{D}$ and; 2) To marginalize with respect to the unknown ``true'' parameters $\bm \theta$. We also exploit the conditional independence $ Y \indep \mathcal{D} \mid X, \bm \theta$, and the model for making new predictions is expressed as 
\begin{align}
