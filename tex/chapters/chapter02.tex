...
The most natural strategy is to approximate the normalizing factor with Monte Carlo estimation. However, this requires to sample from the energy based model which is not immediate either. The solution is to resort to sampling strategies that works with unnormalised distributions such as MCMC techniques.
