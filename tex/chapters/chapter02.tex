...
\begin{figure}
  \centering
  \includegraphics{figures/chapter02/ch2_train_test_error.pdf}
  \caption{This plot is taken from \citet{louppe2014understanding}. It shows the necessary trade-off between the model's complexity and generalization performance. As the model becomes more complex it starts overfitting the trainin data and the test performance decreases.}
...
\node[simple, above=of b]  (Y_T) {$\mathcal{N}(\mathbf{0}, I)$};
...
\node[simple, below=of b]  (Z) {$\approx \mathcal{N}(\mathbf{0}, I)$};
