...
Assuming iid data, we can express the learning task as a generic optimization problem:
\begin{align}
...
The formulation in \ref{eq:learn_BN} does not say much about concrete strategies to fit the parameters of a BN. In practice, we often separate structure learning from distribution fitting. Structure learning is a combinatorial problems whereas fitting the parameters of conditional distributions is a continuous optimization problem, solving both problems jointly is an open problem. In \Cref{ch:06}, we will see one strategy to formulate the combinatorial problem into a continuous one, these are advanced concepts that are beyond the scope of this background. Now, we focus on the two sub-problems independently.
...
\end{align}
The choice of the parameterisation and the prior $\pi$ are key to find a good model when $N$ is finite. Although there exist parametric universal density approximators such as normalizing flows, learning the parameters of these functions can be challenging when the dataset is small or the phenomenon modeled complex. The optimization problem described by \eqref{eq:learn_factors} is usually solved via stochastic gradient descent nowadays.

\paragraph{Structure learning}
It is sometimes difficult to define a relevant structure a priori and it may be interesting to learn it from data instead. The purpose of structure learning is to find a sparse topology that does not imply (conditional) independence in contradiction with the data. We can formulate this search as an optimization problem under constraints in the space of DAGs $\mathcal{G}$:
\begin{align}
...
\end{align}
where $A(G)$ is the adjacency matrix of the graph $G$, $I(G)$ the set of (conditional) independence encoded by $G$, and $I(\mathcal{D})$ the set of independence observed in the data. The constraints ensure that only independence observed in the data are encoded in the structure and we aim for the minimum number of edges. This problem is combinatorial, however we can relax it by fixing the topological ordering in advance. There exists a greedy algorihm that solves the relaxed problem. The quality of the solutions with respect to \eqref{eq:learn_structure} depends on the chosen ordering. In addition to its intractability, the second limitation of the formulation in \eqref{eq:learn_structure} is the lack of prior knowledge. A partial solution is express the prior as binary variable that represent whether a structure is plausible or not. Removing these unikely structures from the search space $\mathcal{G}$ would take this prior. However expressing more subtle priors over graphs and handling them efficiently in the scope of structure learning is still an open research question.

The evaluation of $I(\mathcal{D})$ is key for structure learning. In theory testing for independence in the data is impossible as it would require to access the (conditional) joint distribution between the two variables tested, which is what we are trying to learn. In practice, we create statistical tests of independence by making assumptions on the class of interactions and marginal distributions (e.g. linear gaussian, discretization of variables, etc.). These tests are reasonably accurate for unconditional independence but do not scale well to conditional tests. It is interesting to note that while exact independence between continuous variables is impossible to prove formally provided a finite dataset, it is often beneficial to considere weakly dependent variables as independent. Indeed this usually simplifies the model and improves its generalization to unseen data.

In parallel to these three generic formulations (\ref{eq:learn_BN},~\ref{eq:learn_factors},~\ref{eq:learn_structure}) there exist many specialized algorithms. Some have proposed greedy algorithms which provide useful approximation of the true solution~\citep{}. Others have focused on sub-class of BNs to provide more efficient algorithms~\citep{}. In general topology learning per itself is very hard. Learning the distributions at the same time leads sometimes to simpler formulation however generic and efficient algorithms to solve \eqref{eq:learn_BN} does not exist yet. This problem has recently regained great attention from the machine learning community in the context of causal discovery~\citep{}.
