...
These models have progressively received less attention from the machine learning community in favor of other methods that scale better with the dimensionality of the data. %Moreover, practical applications for continuous data stands on a strong hypothesis on the class of distributions combined with linear dependence relationships that limits the modelling capacities.
...
% %We have shown that some one-step normalizing flows' conditioners corresponds to modelling a distribution with a BN with pre-defined topology.
